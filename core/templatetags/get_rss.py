# -*- coding: utf-8 -*-
# http://www.djangosnippets.org/snippets/852/
# with small fixes for using url_var_name as variable with new Django
...
class RssParserNode(template.Node):
    def __init__(self, var_name, url=None, url_var_name=None):
        self.url = url
        self.url_var_name = template.Variable(url_var_name)
        self.var_name = var_name

    def render(self, context):
        if self.url:
            url = self.url
            context[self.var_name] = feedparser.parse(url)
        else:
            try:
                context[self.var_name] = feedparser.parse(self.url_var_name.resolve(context))
            except KeyError:
